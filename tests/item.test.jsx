...
describe('Getting product /shoppingitem/list', () => {
    ...
    test('Return list of products with content parametr', async () => {
        ...
        const response = await request(app)
            ...
            .query({
                content:
                    item[faker.number.int({ min: 0, max: item.length })].slug,
            })
        expect(response.status).toBe(200)
        expect(response.body.message).toBeUndefined()
    })
    ...
    test('Return product not found', async () => {
        const response = await request(app).get(
            `/shoppingitem/list/${faker.lorem.word()}`,
        )
        expect(response.status).toBe(400)
        expect(response.body.message).not.toBeUndefined()
    })
...
describe('Updating product /shoppingitem/update', () => {
    test('Updating product', async () => {
        ...
        const response = await request(app)
            .put(`/shoppingitem/update/${item.slug}`)
            .send(newProduct)
    ...
    test('Updating non existed product', async () => {
        const product = generateRandomItem()
        const response = await request(app)
            .put(`/shoppingitem/update/${faker.lorem.slug()}`)
            .send(product)
...
describe('Deleting product /shoppingitem/delete', () => {
    ...
    test('Deleteting non-existing product', async () => {
        const response = await request(app).delete(
            `/shoppingitem/delete/${faker.lorem.slug()}`,
        )
        expect(response.status).toBe(400)
        expect(response.body.message).not.toBeUndefined()
    })
